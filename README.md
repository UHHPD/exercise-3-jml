...
to 10 occur in the file. Include the corresponding header file with
#include<vector>
using namespace std;
Create a vector object with eleven
entries: vector<int> zaehler(11); Then read all $N = 234$
numerical values $n_i$ from datensumme.txt and always increase the
...
terminal and entering in the program
plot "./hist.txt"  smooth freq with boxes
(Use quit to exit gnuplot.) Alternatively, you can use an
...
set output 'out.svg'
plot  "data.txt" using 1:2 smooth freq with boxes
Write a function double poisson(double mu, int k) that computes the
Poisson probability $P(k,\mu) = \frac{\mu^k e^{-\mu}}{k!}$.
Use the function double exp(double x) from cmath as the
implementation of the exponential function and the function
...
cmath).
Now write a new file histpoi.txt, which contains in a third
column the expectation from the Poisson distribution
...
We also want to quantify the agreement with the Poisson hypothesis.
For this we use the log-likelihood function
...
values $n_i$: $\mathcal{L}(\mu) = \prod_{i=1}^{234} P(n_i,\mu)$,
assuming the Poisson distribution hypothesis is correct. To do this,
store all values from "datensumme.txt" in a vector std::vector<int>
daten.
Use daten.push_back(zahl) to add the value of the variable
zahl to the vector.
Now write a function
double prob(std::vector<int> daten, double mu) that computes the
likelihood, i.e. the probability
$\mathcal{L}(\mu) = \prod_i \frac{\mu^{k_i}e^{-\mu}}{k_i!}$ of
obtaining the data for a given $\mu$.
To iterate over the values in daten, use:
for(int k : daten) {
  ...
}
Print the probability for $\mu = 3.11538$ (mean of the sample).
...
Scan $\mu$ with a step size of 0.1. Display the values as a graph
by starting gnuplot in the terminal and entering in the program
plot "likelihood.txt" with line. (Use quit to exit
gnuplot.)
Decrease the step size to obtain a nicer plot. What do you see?
Find the best estimate for $\mu$ from the maximum of the likelihood.
(With plot [xmin:xmax] "likelihood.txt" with line you can display
only a section in $x$ of the graph.)
Now write a second file "nll.txt" with the value pairs $\mu$
and $- 2\ln \mathcal{L}(\mu)$, the negative log-likelihood. Use
the log function from cmath. Also create a graph for these
values with gnuplot.
...
log-likelihood. Determine the error on the estimated $\mu$ value by
finding the interval in which
...
Poisson distribution with $\mu = 3.11538$ by the best possible Poisson
probability, namely the one obtained when for each individual value
$\mu = n_i$ is assumed. Compute the likelihood ratio:
$\Lambda = \prod_{i=1}^{234} P(n_i,\mu)/\prod_{i=1}^{234} P(n_i,n_i)$
and print the value.
If the Poisson distribution hypothesis is correct, then
$-2\ln \Lambda$ should follow a
$\chi^2$ distribution
with $n_\textrm{dof} = 233$ degrees of freedom.
For a number of degrees of freedom greater than 100, the
$\chi^2$ distribution should be approximately a normal distribution
with mean $n_\textrm{dof}$ and standard deviation
$\sqrt{2n_\textrm{dof}}$. Compute the relative deviation of your
likelihood ratio from the mean:
$z = \frac{2\ln \Lambda - n_\textrm{dof}}{\sqrt{2n_\textrm{dof}}}$.
